...
const jsonData = [
    ...
    {
        "id": "gingko-avenue",
        "backgroundColor": "oklch(23.0% 0.020 75.0 / 1)",
        "mainColor": "oklch(82.0% 0.155 95.0 / 1)",
        "secondaryColor": "oklch(62.0% 0.085 55.0 / 1)"
    },
    {
        "id": "bamboo-shadow",
        "backgroundColor": "oklch(18.0% 0.040 150.0 / 1)",
        "mainColor": "oklch(72.0% 0.135 145.0 / 1)",
        "secondaryColor": "oklch(60.0% 0.090 135.0 / 1)"
    },
    {
        "id": "yukata-blue",
        "backgroundColor": "oklch(19.0% 0.035 250.0 / 1)",
        "mainColor": "oklch(70.0% 0.120 240.0 / 1)",
        "secondaryColor": "oklch(75.0% 0.070 200.0 / 1)"
    },
    {
        "id": "neon-sakura",
        "backgroundColor": "oklch(18.5% 0.055 320.0 / 1)",
        "mainColor": "oklch(82.0% 0.210 340.0 / 1)",
        "secondaryColor": "oklch(72.0% 0.165 285.0 / 1)"
    },
    {
        "id": "bamboo-mist",
    ...
    {
        "id": "ramune-fizz",
        "backgroundColor": "oklch(92.0% 0.030 210.0 / 1)",
        "mainColor": "oklch(62.0% 0.145 205.0 / 1)",
        "secondaryColor": "oklch(70.0% 0.105 185.0 / 1)"
    },
    {
        "id": "lantern-blue",
        "backgroundColor": "oklch(16.0% 0.030 250.0 / 1)",
        "mainColor": "oklch(75.0% 0.115 230.0 / 1)",
        "secondaryColor": "oklch(65.0% 0.085 255.0 / 1)"
    },
    {
        "id": "citrus-shrine",
    ...
    {
        "id": "noh-mask",
        "backgroundColor": "oklch(20.0% 0.028 285.0 / 1)",
        "mainColor": "oklch(88.0% 0.045 90.0 / 1)",
    ...
    {
        "id": "storm-lake",
        "backgroundColor": "oklch(15.0% 0.050 250.0 / 1)",
        "mainColor": "oklch(78.0% 0.180 235.0 / 1)",
        "secondaryColor": "oklch(62.0% 0.130 270.0 / 1)"
    },
    {
        "id": "winter-kimono",
    ...
    {
        "id": "ink-wash",
        "backgroundColor": "oklch(96.0% 0.006 90.0 / 1)",
        "mainColor": "oklch(30.0% 0.015 260.0 / 1)",
        "secondaryColor": "oklch(55.0% 0.020 260.0 / 1)"
    },
    {
        "id": "plaza-snow",
...
const originalThemes = ["dark", "light", "terminal", "rose", "ocean", "miami", "bento", "serika-dark", "matrix", "tokyo-night"];

// 1. Update ThemeContext.tsx
const contextFile = "lib/contexts/ThemeContext.tsx";
let contextCode = fs.readFileSync(contextFile, "utf-8");

const allThemeTypes = [...originalThemes, ...jsonData.map(t => t.id)].map(id => `"${id}"`).join(" | ");

contextCode = contextCode.replace(
    /export type Theme = .*?;/s,
    `export type Theme = ${allThemeTypes};`
);
fs.writeFileSync(contextFile, contextCode);

// 2. Update globals.css
const cssFile = "app/[locale]/globals.css";
let cssCode = fs.readFileSync(cssFile, "utf-8");

const START = "/* THEMES_START */";
const END = "/* THEMES_END */";

let blocks = "";
for (const t of jsonData) {
    let surface = t.backgroundColor;
    let hover = t.backgroundColor;

    // Manual parse instead of regex to be safe
    const clean = t.backgroundColor.replace('oklch(', '').replace(' / 1)', '').trim();
    const parts = clean.split(' ');
    if (parts.length >= 3) {
        const l = parseFloat(parts[0].replace('%', ''));
        const c = parts[1];
        const h = parts[2];
        const delta = l > 50 ? -3 : 3;
        surface = `oklch(${(l + delta).toFixed(1)}% ${c} ${h} / 1)`;
        hover = `oklch(${(l + delta * 2).toFixed(1)}% ${c} ${h} / 1)`;
    }

    blocks += `
[data-theme="${t.id}"] {
  --background: ${t.backgroundColor};
  --foreground: ${t.mainColor};
  --primary: ${t.secondaryColor};
  --secondary: ${t.mainColor};
  --surface: ${surface};
  --surface-hover: ${hover};
...
const themed = `${START}\n${blocks}\n${END}`;
if (cssCode.includes(START)) {
    // Robust replacement without new RegExp escape hell
    const before = cssCode.split(START)[0];
    const after = cssCode.split(END)[1] || "";
    cssCode = before + themed + after;
} else {
    cssCode = themed + "\n\n" + cssCode;
}
fs.writeFileSync(cssFile, cssCode);

// 3. Update ThemeSwitcher.tsx
const switcherFile = "components/Shared/ThemeSwitcher.tsx";
let switcherCode = fs.readFileSync(switcherFile, "utf-8");

let newItems = "";
for (const t of jsonData) {
    const label = t.id.split("-").map(w => w[0].toUpperCase() + w.slice(1)).join(" ");
    newItems += `        { id: "${t.id}", label: "${label}", icon: <Palette size={14} />, color: "bg-[${t.backgroundColor.split(' ').join('_')}] border border-transparent" },\n`;
}

// Find the themes array and inject before the closing bracket
const themesArrayStart = "const themes: { id: Theme; label: string; icon: React.ReactNode; color: string }[] = [";
const themesArrayEnd = "    ];";

if (switcherCode.includes(themesArrayStart)) {
    const parts = switcherCode.split(themesArrayStart);
    const endParts = parts[1].split(themesArrayEnd);
    // endParts[0] contains original items
    // Rebuild
    switcherCode = parts[0] + themesArrayStart + endParts[0] + newItems + "    ];" + endParts.slice(1).join(themesArrayEnd);
    fs.writeFileSync(switcherFile, switcherCode);
} else {
    console.log("Could not find themes array in ThemeSwitcher.tsx");
}
